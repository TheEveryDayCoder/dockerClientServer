...
int main(){
 ...
 //udp port 21623
 printf("The ServerC is up and running using UDP on port 23623\n");
 getaddrinfo(NULL,"23623",&hints,&res);
//construct udp send socket
 int socket_desc;
    ...
    isBinded=bind(socket_desc,res->ai_addr,res->ai_addrlen);
     ...
     while(proc){
       ...
       else
       {
         ...
         if(udp_out<0)
         {
           printf("ERROR: UDP message to server C  was not sent out. \n");
         }
         printf("The ServerC finished sending the response to the Main Server.\n");
         }
         else if (containsTXLIST(buf)){ //performtxList
           printf("The ServerC received a request from the Main Server.\n");
           char fileName[]="block3.txt";
         FILE *fp=fopen(fileName,"r");
         int ii=0;
        ...
        else
        {
          ...
          printf("my Check Buffer: \"%s\" with out of: %i\n",buffer,out); 
          int pass=fprintf(fp,"\n%s\n",buffer); fclose(fp);
          char msg[1];
